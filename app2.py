...
@app.route('/coun')
def coun():
    return render_template('coun.html', language=get_locale())  # تأكد من أن هذا الملف موجود
...
def index1():
    ...
    return render_template('index1.html', language=get_locale(), student=student_data, profile_image=profile_image)
...
# ---------------------------------------- الجزء الخاص بتحميل أعلى و أدنى نقطة للطلاب في صفحة index1 ---------------------------------------------
...
BASE_DIR = os.path.abspath(os.path.dirname(__file__))  # مسار المشروع
UPLOAD_FOLDER = os.path.join(BASE_DIR, 'static', 'uploads')  # مسار كامل للمجلد
